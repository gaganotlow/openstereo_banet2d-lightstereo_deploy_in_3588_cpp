...
{
  auto engine =
      CreateOrtInferCore("/workspace/models/lightstereo_s_orin_epoch199_best_val_epe_480x640.onnx",
                         {{"left_img", {1, 3, 480, 640}}, {"right_img", {1, 3, 480, 640}}},
                         {{"disp_pred", {1, 1, 480, 640}}});
  auto preprocess_block = CreateCpuImageProcessingResizePad(
      ImageProcessingPadMode::TOP_RIGHT, ImageProcessingPadValue::EDGE, true, true,
      {123.675, 116.28, 103.53}, {58.395, 57.12, 57.375});

  return CreateLightStereoModel(engine, preprocess_block, 480, 640, {"left_img", "right_img"},
                                {"disp_pred"});
}
...
std::shared_ptr<BaseStereoMatchingModel> CreateLightStereoRknnModel()
{
  auto engine = CreateRknnInferCore(
      "/workspace/models/lightstereo_s_orin_epoch199_best_val_epe_480x640.rknn",
